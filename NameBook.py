class BaseContact:
    def __init__(self, name, last_name, adress, email, phone_number=None):
        self.name = name
...
class BusinessContact(BaseContact):
    def __init__(self, position, company_name, bussines_phone, company_email, name, last_name, adress, email,
                 phone_number):
        super().__init__(name, last_name, adress, email, phone_number)
...
def create_contacts(contact_type='Base', quantity='1'):
    ...
    if contact_type == 'Base':
        for item in range(quantity):
            item = fake.last_name()
            item = BaseContact(name=fake.first_name(),
...
print(contacts[1])
contacts[1].label_lenght()
contacts[1].contact()
print(dir(contacts[1]))
BaseContact.costam()